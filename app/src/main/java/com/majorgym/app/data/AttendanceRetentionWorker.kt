 ...
 * alongside - not instead of - [MembershipHoldWorker.schedule]) and
 * deletes attendance records older than [ATTENDANCE_RETENTION_MONTHS].
 *
 * Deliberately separate from [MembershipHoldWorker]: that worker's job is the
 * Hold Members ACTIVE/HOLD lifecycle transition, and its timing/conditions
 * must not change (Change 5). This worker does one small, unrelated thing -
 * a plain indexed range delete on attendance_records - and nothing else. It
 * never touches members, attendance recording, QR/fingerprint check-in
 * logic, or Morning/Evening grouping.
 *
